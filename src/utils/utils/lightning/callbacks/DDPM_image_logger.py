...
class DDPM_image_logger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        if self.check_frequency(batch_idx) and self.max_images > 0:
            ...
            with torch.no_grad():
                sample_shape = [self.max_images]+list(batch['im'].shape[1:])
                images = pl_module.p_sample_loop(sample_shape)
                images = {'sampled_image':images}
